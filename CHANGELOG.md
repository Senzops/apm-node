...
feat(ai): auto-instrument the new unified `@google/genai` SDK (the `GoogleGenAI` client — `ai.models.generateContent`/`generateContentStream`/`embedContent` and `ai.chats.create(...).sendMessage(...)`/`sendMessageStream(...)`, including the automatic-function-calling loop). Previously only the legacy `@google/generative-ai` and `@google-cloud/vertexai` packages were covered; apps on the new SDK now get first-class AI generations (model, tokens, latency, cost, finish reason) with zero code beyond `Senzor.init()`, plus Gemini "thinking" reasoning tokens (`usageMetadata.thoughtsTokenCount`), streaming usage + time-to-first-token (captured without consuming the stream), and opt-in prompt/output capture. Same `google-genai` instrumentation key. Instruments the `Models.*Internal` chokepoints, so chat/direct/streaming each record exactly once.

# 1.5.1

fix(ai): a failing tool / MCP / structural observation no longer marks the whole AI trace as failed. Only a model call (LLM generation / embedding) erroring — or the workflow root throwing — fails the trace; a handled tool/MCP failure is normal agent behaviour and is recorded on its own span (and reliability metrics) without flipping the trace status. `Senzor.ai.generation()` and the internal agent-loop recorder now taint the trace only for model-call errors; `Senzor.ai.agent/tool/mcp` scopes no longer taint the trace on a handled error — a fatal scope failure still surfaces by throwing, which propagates to the enclosing `trace()`/`agent()` and fails it as expected. MCP `CallToolResult` `isError` results are treated the same way (error span, trace unaffected unless it is the model outcome).
